...
const emits = defineEmits(['user-set-success']);
...
const submitForm = async () => {
    ...
    try {
        const addedUser: User = await UserService.addUserToServer(user);
            emits('user-set-success', addedUser);
    } catch (error) {
...
<template>
    <div class="max-w-md mx-auto mt-4">
        ...
        <form @submit.prevent="submitForm">
            <div class="mb-4">
                <label
                for="name" class="block text-gray-700 font-bold mb-2"> {{ t('userFormName') }}</label>
                <input
            ...
            <div class="mb-4">
                ...
                <span v-if="introError" role="alert" class="text-red-500 text-xs italic">{{ $t('userFormIntroError')
                }}</span>
            </div>
            <div class="mt-6">
